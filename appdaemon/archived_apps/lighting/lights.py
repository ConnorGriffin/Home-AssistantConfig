...
class Lights(hass.Hass):
    ...
    def initialize(self):
        ...
        if "entities" in self.args:
            ...
            for entity in self.args["entities"]:
                ...
                on_threshold = entity.get(
                    'on_threshold', self.args['on_threshold'])
                off_threshold = entity.get(
                    'off_threshold', self.args['off_threshold'])
                ...
                # Presence detection
                if motion_sensor:
                    self.log("Monitoring {} for presence changes.".format(
                        light_friendly), "INFO")
                    self.listen_state(
                ...
                if humidity_sensor and humidity_threshold:
                    self.log("Monitoring {} for humidity threshold of {}%.".format(
                        light_friendly, humidity_threshold), "INFO")
                    self.listen_state(
                        callback=self.humidity_cb,
    ...
    def mode_dropdown_cb(self, entity, attribute, old, new, kwargs):
        ...
        light_entity = entity.replace(
            'input_select', 'light').replace('_mode', '')
    ...
    def restore_group_cb(self, entity, attribute, old, new, kwargs):
        ...
        self.log('Resetting {} entities (unhiding light).'.format(
            self.friendly_name('group.{}'.format(group))))
        self.call_service(
            service='group/set',
            object_id=group,
    ...
    def alarm_fired_cb(self, event_name, data, kwargs):
        ...
        if override != 'alarm':
            ...
            self.log('{}: Turned on by alarm {}.'.format(
                self.friendly_name(light_entity), alarm_name))
            self.set_override(
                entity_id=light_entity,
                override='alarm',
                brightness_pct=setting['max_brightness']
            ...
            for group in hide_switch_groups:
                ...
                self.log('Hiding {} from {}.'.format(self.friendly_name(
                    light_entity), self.friendly_name(group_id)))
    ...
    def auto_brightness_cb(self, kwargs):
        ...
        if check_current_brightness:
            current_brightness = self.get_state(
                entity_id, attribute='brightness')
        ...
        if (state == 'on' or source in ['turned_on_cb', 'set_override', 'presence']) and not setting['override']:
            ...
            for entity in self.args['entities']:
                if entity['name'] == entity_id.split('.')[1]:
                    schedule = entity.get(
                        'brightness_schedule', self.args['brightness_schedule'])
            ...
            if target_percent:
                ...
                if last_percent != target_percent:
                    if check_current_brightness and abs(last_percent - current_brightness_pct) > 5:
                        self.log(
                            '{}: Brightness changed manually, not moving'.format(friendly_name))
                        self.call_service(
                            service='input_select/select_option',
                            entity_id=mode_entity,
                            option='Manual'
                        ...
                        self.log("{}: Setting auto-brightness - {}% over {} seconds".format(
                            friendly_name, round(target_percent, 2), transition))
                        self.turn_on(
                            entity_id,
    ...
    def presence_cb(self, entity, attribute, old, new, kwargs):
        ...
        if new == 'on' and light_state == 'off':
            ...
            if self.get_state('input_boolean.light_brightness_automation') == 'on':
                self.auto_brightness_cb(
                    dict(entity_id=light_entity, source='presence'))
            else:
            ...
            if setting['override'] == 'showering':
                ...
                if self.get_state('device_tracker.diana_pixel2') == 'not_home':
                    ...
                    self.log('{}: Turning off when humidity drops below threshold.'.format(
                        light_friendly))
            else:
                # Testing only, don't auto turn-off if Diana is home
                if self.get_state('device_tracker.diana_pixel2') == 'not_home':
                    self.turn_off_and_refresh(light_entity)
                    self.log('{}: Turning off, no presence detected.'.format(
                        light_friendly))
        elif new == 'on' and light_state == 'on' and setting['override'] == 'showering' and setting['next_action'] == 'turn_off':
            setting['next_action'] = None
            self.log('{}: Cancelling next action (humidity turn off), presence detected.'.format(
                light_friendly))
        ...
        if float(new) >= humidity_threshold and setting['override'] != 'showering':
            ...
            if light_state == 'off':
                ...
                if self.get_state('input_boolean.light_brightness_automation') == 'on':
                    ...
                    self.turn_on_and_refresh(light_entity)
            self.log('{}: Shower is running, turning on lights and ignoring motion detection.'.format(
                light_friendly))
            ...
            self.log('{}: Shower is no longer running, resuming motion detection.'.format(
                light_friendly))
