...
class DoorAutolock(hass.Hass):
    ...
    def initialize(self):
        for door in self.args.get("doors"):
            ...
            if sensor and lock:
                unlocked_seconds = door.get(
                    'unlocked_seconds', self.args['unlocked_seconds'])
                closed_seconds = door.get(
                    'closed_seconds', self.args['closed_seconds'])

                # Listen for each door to be closed and unlocked for a certain amount of time
                self.listen_state(
                    callback=self.unlocked_cb,
                    entity=lock,
                    new='unlocked',
                    immediate=True,
    ...
    def closed_cb(self, entity, attribute, old, new, kwargs):
        ...
        now = datetime.utcnow()
        lock_state_time = self.convert_utc(
            lock_state['last_changed']).replace(tzinfo=None)
    ...
    def unlocked_cb(self, entity, attribute, old, new, kwargs):
        ...
        now = datetime.utcnow()
        door_state_time = self.convert_utc(
            door_state['last_changed']).replace(tzinfo=None)
